...
const SortList = ({ closeModalSort, events }) => {
  return (
    <div className={s.modalBackdropSort}>
        <div className={s.modalContentSort}>
                  
        <button aria-label="close sort list" className={s.closeButtonSort} onClick={closeModalSort}>
        ...
        <ul className={s.sort__list}>
            <li className={s.sort__listItem}>
                <button aria-label="sort by name up" className={s.sort__listBtn} onClick={() => { events.sort((a, b) => a.title.localeCompare(b.title)); closeModalSort(); }}
                    >by name <svg width="24" height="24"><use href={`${icon}#icon-shapeup`}/></svg>
                </button>
            </li>
                  
            <li className={s.sort__listItem}>
                <button aria-label="sort by name down" className={s.sort__listBtn} onClick={() => { events.sort((a, b) => b.title.localeCompare(a.title)); closeModalSort(); }}
                      >by name <svg width="24" height="24"><use href={`${icon}#icon-shapedown`}/></svg>
                </button>
            </li>
                  
            <li className={s.sort__listItem}>
                <button aria-label="sort by data up" className={s.sort__listBtn} onClick={() => {
                    events.sort((a, b) => {
            ...
            <li className={s.sort__listItem}>
                <button aria-label="sort by data down" className={s.sort__listBtn} onClick={() => {
                      events.sort((a, b) => {
                        const [dayA, monthA] = a.date.split('.').map(Number);
                        const [dayB, monthB] = b.date.split('.').map(Number);
            ...
            <li className={s.sort__listItem}>
                <button aria-label="sort by priority up" className={s.sort__listBtn} onClick={() => {
                        events.sort((a, b) => {
            ...
            <li className={s.sort__listItem}>
                <button aria-label="sort by priority down" className={s.sort__listBtn} onClick={() => {
                      events.sort((a, b) => {
                        const priorityOrder = { "high": 3, "medium": 2, "low": 1 };
